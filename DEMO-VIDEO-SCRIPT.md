...
1. Load `demo.carrillodynamics.com` and `hvac.carrillodynamics.com` (or hub preview).
2. Browser zoom ~120%. Hide bookmarks. Quiet room.
3. EN language on for the walk-through; flip to ES once at the bilingual beat.
4. Have end-card text ready (Canva / PowerPoint / Notion full-screen).

---

## Shot list + VO (English + Spanish closer)
...
## End card (on screen the whole closing)

```
CARRILLO DYNAMICS
Book a 15-min strategy call
carrillodynamics.com/start
```

---

## Optional later (not tomorrow)

Full Spanish remake of the same arc, ending on the same URL. Only worth it if `/start` ES traffic is meaningful.

## After record

1. Export 1080p.
2. Upload YouTube **Unlisted** or Loom.
3. Paste the embed URL into `/start` (replace the “coming soon” block).
